...
(t/deftest dep->repository-opts-test
  (t/is (= {:repositories sut/default-repos
            :snapshots? false}
           (sut/dep->repository-opts (r/map->Dependency {:version "1.0.0"}))))
  (t/is (= {:repositories (assoc sut/default-repos
                                 "foo" {:url "s3://foo"})
            :snapshots? true}
           (sut/dep->repository-opts (r/map->Dependency {:repositories {"foo" {:url "s3p://foo"}}
